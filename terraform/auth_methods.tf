resource "vault_auth_backend" "approle_databases" {
  type      = "approle"
  path      = "approle"
  namespace = "${vault_namespace.admin.path}/${vault_namespace.databases.path}"
}

resource "vault_auth_backend" "approle_pki_internal" {
  type      = "approle"
  path      = "approle"
  namespace = "${vault_namespace.admin.path}/${vault_namespace.pki_internal.path}"
}

resource "vault_auth_backend" "approle_ssh" {
  type      = "approle"
  path      = "approle"
  namespace = "${vault_namespace.admin.path}/${vault_namespace.ssh.path}"
}
...
resource "vault_jwt_auth_backend" "jwt_encryption" {
  namespace           = "${vault_namespace.admin.path}/${vault_namespace.encryption.path}"
  path                = "jwt"
  jwks_url            = "https://example.com/.well-known/jwks.json"
  bound_issuer        = "https://example.com/"
}
...
resource "vault_auth_backend" "kubernetes" {
  type      = "kubernetes"
  path      = "kubernetes"
  namespace = "${vault_namespace.admin.path}/${vault_namespace.kubernetes.path}"
}
...
resource "vault_auth_backend" "tls_pki_web" {
  type      = "cert"
  path      = "cert"
  namespace = "${vault_namespace.admin.path}/${vault_namespace.pki_web.path}"
}
...
resource "vault_auth_backend" "tls_x509" {
  type      = "cert"
  path      = "cert"
  namespace = "${vault_namespace.admin.path}/${vault_namespace.x509.path}"
}

# OIDC Auth
resource "vault_jwt_auth_backend" "oidc_cloud" {
  namespace           = "${vault_namespace.admin.path}/${vault_namespace.cloud.path}"
  path                = "oidc"
  type                = "oidc"
...
resource "vault_auth_backend" "userpass_batch" {
  type      = "userpass"
  path      = "userpass"
  namespace = "${vault_namespace.admin.path}/${vault_namespace.batch.path}"
}

resource "vault_auth_backend" "userpass_security" {
  type      = "userpass"
  path      = "userpass"
  namespace = "${vault_namespace.admin.path}/${vault_namespace.security.path}"
}
